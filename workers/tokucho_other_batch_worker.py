...
class TokuchoOtherBatchWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            # サービス生成
            # =====================================
            pdf_reader = TokuchoOtherPdfReader()
            ledger_writer = LedgerWriter()
            excel_writer = TokuchoOtherExcelWriter()
            ...
            for index, pdf_path in enumerate(
                ...
                # PDF解析前は表示用にファイル名を保持する。
                # 業務委託計画書Noと出力ファイル名は、
                # PDF解析後に取得した値から設定する。
                voucher_no = pdf_path.stem.strip()
                estimate_no = ""
                ...
                try:
                    ...
                    # ExcelのC25には、番号がある場合だけ転記する。
                    # 空欄の場合はTokuchoOtherExcelWriter側でも転記しない。
                    data.voucher_no = application_no

                    # 画面の処理結果では、番号なしを「-」で表示する。
                    voucher_no = (
                        application_no
                        if application_no
                        else "-"
                    ...
                    # 出力ファイル名を生成する。
                    file_name_base = (
                        excel_writer.create_output_file_name(
                            department=(
                                excel_writer.get_department(data)
                            ),
                            work_item=(
                                excel_writer.get_first_work_item(data)
                            ),
                        )
                    )
                    ...
                    # 既存ファイルは残す
                    # =====================================
                    # TokuchoOtherExcelWriter側で同名を確認し、
                    # 「_2」「_3」…を付けて新規保存する。
                    # 既存ファイルの退避・上書きは行わない。
                    ...
                    # Excel・PDF出力
                    # =====================================
                    output_result = excel_writer.write(
                        output_folder=(
