...
別のブランチ名が表示されている場合は、ブランチ名をクリックして `main`
を選択してください。
...
`.env` は、OpenAIやMicrosoft
OneDrive／SharePointへ接続するための設定ファイルです。
...
管理者から設定値を入力するよう案内された場合のみ、`.env`
をメモ帳で開いて編集してください。

> \[!CAUTION\] `OPENAI_API_KEY`
> などの設定値は、外部へ公開しないでください。
>
...
URLは手入力するほか、クリップボードにコピーした状態で **「貼り付け」**
ボタンをクリックして入力できます。
...
処理結果一覧の行を**ダブルクリック**すると、「処理結果詳細」が表示されます。

この操作は、**特調TB・特調以外TB・TG・MSRのすべてのタブ**で使用できます。

詳細画面では、業務に応じて識別番号、見積／請求番号、結果、処理内容、エラーや確認事項、出力ファイル名、管理台帳への記入結果などを確認できます。

一覧で「NG」または「失敗」と表示された場合は、対象行をダブルクリックして詳細を確認してください。
...
成果物名が長い場合や複数行で記載されている場合は、見積書の成果物欄に合わせて表示内容を自動調整します。

同じ成果物名が複数あり、2行目の内容が異なる場合は、内容を区別できるよう2行目も使用します。成果物欄に収まる場合は、同名成果物を2行で表示する場合があります。

成果物数が多く2行表示では欄に収まらない場合は、必要に応じてAIを使用して1行に短縮します。

> \[!IMPORTANT\]
> AIによる短縮が行われた場合も、出力後に成果物の意味が正しく保たれていることを確認してください。

### 成果物表の「備考」について

特調TBの業務計画書では、「成果物名称」と右側の「備考（数量／サイズ指定など）」を別の項目として扱います。

見積書の成果物欄には**「成果物名称」の内容だけ**が出力されます。備考欄に記載されている「一式」「一式／一任」「1/A3」「数量、サイズ指定無」などは、見積書の成果物名には追加されません。

文字情報を持つPDFだけでなく、画像形式のPDFをAIで解析する場合も同じ扱いになります。

### PDFのフォーマットが異なる場合

特調TBとして必要な情報を取得できず、PDFが対応フォーマットと異なる可能性がある場合は、処理結果が「失敗」となり、詳細画面に確認メッセージが表示されます。

この場合は、処理結果の行をダブルクリックし、「取得できなかった項目」「対象PDF」「処理種別」を確認してください。
...
特調以外TBでも、成果物名が長い場合や改行されている場合は、見積書の表示領域に合わせて内容を調整します。同名成果物の区別や表示行数の調整が必要な場合は、2行目の内容やAIによる短縮を使用する場合があります。

対応フォーマットと異なる可能性があるPDFの場合は、処理結果詳細にフォーマット確認メッセージが表示されます。

### 帳票のレイアウトが一部異なる場合

特調以外TBでは、業務委託計画書のレイアウトに一部差異がある場合でも、「作業内容」「納期」「計画部署」等の項目を基準に必要情報を取得します。

依頼部署の取得では、帳票内の「承認」「審査」「作成」等の承認欄を部署名として扱わないよう判定します。

処理結果が「失敗」となった場合は、結果行をダブルクリックして詳細を確認してください。必要項目を取得できない場合は、PDFが対応フォーマットであることも確認してください。
...
フォルダ内に仕様書と見積書フォーマットがあることを確認してください。
...
MSR処理で管理台帳を開けず失敗した場合は、処理結果の行をダブルクリックして詳細を確認してください。

管理台帳には別のExcelファイルを参照する「外部リンク」情報が残っている場合があります。外部リンクが存在すること自体は異常ではありませんが、その情報の一部にExcelが不整合を検出すると、ツールから管理台帳を開けない場合があります。

その場合は次の手順で確認します。

1.  OneDriveまたはSharePoint上のMSR管理台帳をMicrosoft
    Excelで直接開きます。
2.  「ファイルに問題が見つかりました」などの修復確認が表示された場合は、内容を確認して修復します。
3.  修復後の管理台帳を上書き保存します。
4.  Excelを閉じます。
5.  見積書作成ツールでもう一度MSR処理を実行します。

> \[!IMPORTANT\]
> 外部リンクが存在するだけで、必ず問題が発生するわけではありません。Excelで修復メッセージが表示された場合に修復して上書き保存してください。

管理台帳が別の利用者によって開かれている場合、アクセス権限がない場合、Excel側で別の問題が発生している場合も開けないことがあります。
...
各タブの処理結果一覧は、確認したい行を**ダブルクリック**すると「処理結果詳細」が開きます。成功・NG・失敗の詳細確認に使用してください。
...
同じファイルで長時間止まっている場合は、必要に応じて **「処理を中止」**
をクリックします。
...
## 15.8 PDFのフォーマット確認メッセージが表示された

特調TBまたは特調以外TBで「PDFのフォーマットが対応形式と異なる可能性があります。」と表示された場合は、対象行をダブルクリックして詳細を確認してください。

選択した処理種別、取得できなかった項目、対象PDFを確認し、別業務のPDFや対応していない形式のPDFが入力フォルダに入っていないか確認します。

------------------------------------------------------------------------

## 15.9 MSR管理台帳を開けない

MSRで管理台帳を開けない旨のエラーが表示された場合は、対象行をダブルクリックして詳細を確認してください。

外部リンク情報の不整合について案内が表示された場合は、「11.9
MSR管理台帳を開けない場合」の手順に従って確認してください。
...
基本的には、必要な項目を設定して **「一括実行」**
をクリックすれば、見積書作成から管理台帳への登録まで自動で行われます。
